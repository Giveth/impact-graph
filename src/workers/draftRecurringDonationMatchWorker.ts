...
import { DRAFT_RECURRING_DONATION_STATUS, DraftRecurringDonation } from '../entities/draftRecurringDonation';

type DraftRecurringDonationWorkerFunctions = 'matchDraftRecurringDonations';

export type DrafRecurringtDonationWorker = WorkerModule<DraftRecurringDonationWorkerFunctions>;
...
const worker: DrafRecurringtDonationWorker = {
  async matchDraftRecurringDonations() {
    ...
    try {
      ...
      while (true) {
        const userIds = await DraftRecurringDonation.createQueryBuilder('draftRecurringDonation')
