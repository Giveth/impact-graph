...
function projectsTestCases() {
  ...
  it('should return projects, sort by project power', async () => {
    ...
    projects = result.data.data.projects.projects;
    assert.equal(projects[projects.length - 3].id, project1.id);
    assert.equal(projects[projects.length - 2].id, project2.id);
    assert.equal(projects[projects.length - 1].id, project3.id);

    assert.equal(projects[0].projectPower.powerRank, 4);
    assert.equal(projects[projects.length - 3].projectPower.powerRank, 3);
    assert.equal(projects[projects.length - 2].projectPower.powerRank, 2);
    assert.equal(projects[projects.length - 1].projectPower.powerRank, 1);
  });
