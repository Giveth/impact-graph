...
describe(
  'projectBySlug QF Round Relations test cases --->',
  projectBySlugQfRoundRelationsTestCases,
);
...
function projectBySlugQfRoundRelationsTestCases() {
  it('should return projectQfRoundRelations when requested as nested field in qfRounds', async () => {
    // Create a QF round
    const qfRound = QfRound.create({
      isActive: true,
      name: 'Nested Relations Test QF Round',
      slug: `nested-relations-test-qf-round-${Date.now()}`,
      allocatedFund: 200000,
      minimumPassportScore: 8,
      beginDate: new Date(),
      endDate: moment().add(10, 'days').toDate(),
    ...
    const project = await saveProjectDirectlyToDb({
      ...createProjectData(),
      title: 'Nested Relations Test Project',
      slug: `nested-relations-test-project-${timestamp}`,
      adminUserId: user.id,
      statusId: ProjStatus.active,
      reviewStatus: ReviewStatus.Listed,
      verified: true,
    ...
    // Create ProjectQfRound relation
    await createProjectQfRoundRelation(project.id, qfRound.id, 1500, 1);

    // Test the projectBySlug query with nested projectQfRoundRelations
    const result = await axios.post(graphqlUrl, {
      query: fetchProjectBySlugQuery,
      variables: {
        slug: project.slug,
    ...
    assert.isOk(result);
    const projectData = result.data.data.projectBySlug;

    // Verify the project data
    assert.isOk(projectData);
    assert.equal(projectData.id, project.id);
    assert.equal(projectData.title, project.title);

    // Verify qfRounds array
    assert.isOk(projectData.qfRounds);
    assert.isArray(projectData.qfRounds);
    assert.equal(projectData.qfRounds.length, 1);

    const qfRoundData = projectData.qfRounds[0];
    assert.equal(qfRoundData.id, qfRound.id);
    assert.equal(qfRoundData.name, qfRound.name);

    // Verify projectQfRoundRelations is not null
    assert.isOk(qfRoundData.projectQfRoundRelations);
    assert.isArray(qfRoundData.projectQfRoundRelations);
    assert.equal(qfRoundData.projectQfRoundRelations.length, 1);

    const relationData = qfRoundData.projectQfRoundRelations[0];
    assert.isDefined(relationData.sumDonationValueUsd);
    assert.isDefined(relationData.countUniqueDonors);
    assert.isNumber(relationData.sumDonationValueUsd);
    assert.isNumber(relationData.countUniqueDonors);
    assert.equal(relationData.sumDonationValueUsd, 1500);
    assert.equal(relationData.countUniqueDonors, 1);
  ...
  it('should return empty projectQfRoundRelations when no relations exist', async () => {
    // Create a QF round
    const qfRound = QfRound.create({
      isActive: true,
      name: 'Empty Relations Test QF Round',
      slug: `empty-relations-test-qf-round-${Date.now()}`,
      allocatedFund: 100000,
      minimumPassportScore: 8,
      beginDate: new Date(),
      endDate: moment().add(10, 'days').toDate(),
    ...
    const project = await saveProjectDirectlyToDb({
      ...createProjectData(),
      title: 'Empty Relations Test Project',
      slug: `empty-relations-test-project-${timestamp}`,
      adminUserId: user.id,
      statusId: ProjStatus.active,
      reviewStatus: ReviewStatus.Listed,
      verified: true,
    ...
    // Verify projectQfRoundRelations is empty array
    assert.isOk(qfRoundData.projectQfRoundRelations);
    assert.isArray(qfRoundData.projectQfRoundRelations);
    assert.equal(qfRoundData.projectQfRoundRelations.length, 0);
  ...
  it('should return projectQfRoundRelations for multiple QF rounds', async () => {
    // Create two QF rounds
    const qfRound1 = QfRound.create({
      isActive: true,
      name: 'Multi Relations Test QF Round 1',
      slug: `multi-relations-test-qf-round-1-${Date.now()}`,
      allocatedFund: 150000,
      minimumPassportScore: 8,
      beginDate: new Date(),
      endDate: moment().add(10, 'days').toDate(),
    ...
    const qfRound2 = QfRound.create({
      isActive: true,
      name: 'Multi Relations Test QF Round 2',
      slug: `multi-relations-test-qf-round-2-${Date.now()}`,
      allocatedFund: 200000,
      minimumPassportScore: 8,
      beginDate: new Date(),
      endDate: moment().add(15, 'days').toDate(),
    ...
    const project = await saveProjectDirectlyToDb({
      ...createProjectData(),
      title: 'Multi Relations Test Project',
      slug: `multi-relations-test-project-${timestamp}`,
      adminUserId: user.id,
      statusId: ProjStatus.active,
      reviewStatus: ReviewStatus.Listed,
      verified: true,
    ...
    // Create ProjectQfRound relations for both rounds
    await createProjectQfRoundRelation(project.id, qfRound1.id, 800, 1);
    await createProjectQfRoundRelation(project.id, qfRound2.id, 1200, 1);
    ...
    // Verify qfRounds array has both rounds
    assert.isOk(projectData.qfRounds);
    assert.isArray(projectData.qfRounds);
    assert.equal(projectData.qfRounds.length, 2);

    // Sort by ID to ensure consistent order
    const sortedQfRounds = projectData.qfRounds.sort((a, b) => a.id - b.id);

    // Verify first QF round
    const qfRound1Data = sortedQfRounds[0];
    assert.equal(qfRound1Data.id, qfRound1.id);
    assert.equal(qfRound1Data.name, qfRound1.name);
    assert.isOk(qfRound1Data.projectQfRoundRelations);
    assert.isArray(qfRound1Data.projectQfRoundRelations);
    assert.equal(qfRound1Data.projectQfRoundRelations.length, 1);
    assert.equal(
      qfRound1Data.projectQfRoundRelations[0].sumDonationValueUsd,
      800,
    );
    assert.equal(qfRound1Data.projectQfRoundRelations[0].countUniqueDonors, 1);

    // Verify second QF round
    const qfRound2Data = sortedQfRounds[1];
    assert.equal(qfRound2Data.id, qfRound2.id);
    assert.equal(qfRound2Data.name, qfRound2.name);
    assert.isOk(qfRound2Data.projectQfRoundRelations);
    assert.isArray(qfRound2Data.projectQfRoundRelations);
    assert.equal(qfRound2Data.projectQfRoundRelations.length, 1);
    assert.equal(
      qfRound2Data.projectQfRoundRelations[0].sumDonationValueUsd,
      1200,
    );
    assert.equal(qfRound2Data.projectQfRoundRelations[0].countUniqueDonors, 1);
  ...
  it('should return projectQfRoundRelations when requested as direct field on project', async () => {
    // Create a QF round
    const qfRound = QfRound.create({
      isActive: true,
      name: 'Direct Relations Test QF Round',
      slug: `direct-relations-test-qf-round-${Date.now()}`,
      allocatedFund: 180000,
      minimumPassportScore: 8,
      beginDate: new Date(),
      endDate: moment().add(10, 'days').toDate(),
    });
    ...
    const project = await saveProjectDirectlyToDb({
      ...createProjectData(),
      title: 'Direct Relations Test Project',
      slug: `direct-relations-test-project-${timestamp}`,
      adminUserId: user.id,
      statusId: ProjStatus.active,
      reviewStatus: ReviewStatus.Listed,
      verified: true,
    ...
    // Create ProjectQfRound relation
    await createProjectQfRoundRelation(project.id, qfRound.id, 1000, 1);

    // Test the projectBySlug query with direct projectQfRoundRelations field
    const directFieldQuery = `
      query {
        projectBySlug(
          slug: "${project.slug}"
    ...
    // Verify projectQfRoundRelations as direct field
    assert.isOk(projectData.projectQfRoundRelations);
    assert.isArray(projectData.projectQfRoundRelations);
    assert.equal(projectData.projectQfRoundRelations.length, 1);

    const relationData = projectData.projectQfRoundRelations[0];
    assert.isDefined(relationData.sumDonationValueUsd);
    assert.isDefined(relationData.countUniqueDonors);
    assert.isNumber(relationData.sumDonationValueUsd);
    assert.isNumber(relationData.countUniqueDonors);
    assert.equal(relationData.sumDonationValueUsd, 1000);
    assert.equal(relationData.countUniqueDonors, 1);
