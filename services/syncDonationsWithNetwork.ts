...
const numberOfVerifyDonationConcurrentJob = Number(config.get('NUMBER_OF_VERIFY_DONATION_CONCURRENT_JOB')) || 1

verifyDonationsQueue.process(numberOfVerifyDonationConcurrentJob, async (job, done) => {
  ...
  try {
    if (donation.toWalletAddress.toLowerCase() !== donation.project.walletAddress?.toLowerCase()) {
      // TODO now I added this checking in create donation, so after we verified old donations we can remove this checking from here
      donation.verifyErrorMessage = errorMessages.TO_ADDRESS_OF_DONATION_SHOULD_BE_PROJECT_WALLET_ADDRESS
...
export const runCheckPendingDonationsCronJob = () => {
  ...
  schedule(String(config.get('VERIFY_DONATION_CRONJOB_EXPRESSION')), async () => {
