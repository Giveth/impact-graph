...
export class AddBannerEndaomentProjects1724368995904 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        const projects = await queryRunner.query(`SELECT "project"."id", array_agg("category"."value") as "categories","project"."image"
            FROM "project"
        ...
        for(let project of projects) {
            for(let category of project.categories){
                if(category !== 'Endaoment') {
                    ...
                    await queryRunner.query(`UPDATE "project" SET "image" = '${bannerImage}' WHERE "project"."id" = ${project.id};`);
                    if(imageCategoryMapping[category])
                        break;
                }
            }
        }
