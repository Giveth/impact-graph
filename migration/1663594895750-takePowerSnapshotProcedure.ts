...
export class takePowerSnapshotProcedure1663594895750
  implements MigrationInterface
{
  async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`
        CREATE OR REPLACE PROCEDURE PUBLIC."TAKE_POWER_SNAPSHOT"() LANGUAGE 'sql' AS $BODY$
            WITH snapshot_entity as (insert into "power_snapshot" ("time") values (NOW()) returning id)
            insert into "power_percentage_snapshot" ("userId", "projectId", "percentage", "powerSnapshotId")
